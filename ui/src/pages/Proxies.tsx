...
import { Users, Activity, ChevronDown, ChevronRight, Server, Clock, Eye, EyeOff, Plus, Upload, Trash2 } from 'lucide-react';
...
export const Proxies: React.FC = () => {
  ...
  const selectAllProxiesInServer = (serverId: string) => {
    const serverProxies = servers?.find(s => s.id === serverId)?.proxies || [];
    const newSelected = new Set(selectedProxies);
    const allSelected = serverProxies.every(p => newSelected.has(p.id));
    
    if (allSelected) {
      serverProxies.forEach(p => newSelected.delete(p.id));
    } else {
      serverProxies.forEach(p => newSelected.add(p.id));
    }
    setSelectedProxies(newSelected);
  };
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center space-x-3">
            ...
            <button
              ...
              className="ml-3 inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors"
            >
              <Users className="h-4 w-4 mr-2" />
              {showGroupView ? 'Server View' : 'Group View'}
            </button>
          </div>
        </div>
      </div>
            ...
            <div key={server.id} className="bg-white shadow rounded-lg overflow-hidden">
              <div className="p-6">
                ...
                <div className="flex items-center space-x-2 mb-3">
                  <input
                    type="checkbox"
                    checked={server.proxies.length > 0 && server.proxies.every(p => selectedProxies.has(p.id))}
                    onChange={() => selectAllProxiesInServer(server.id)}
                    className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                  />
                  <span className="text-sm text-gray-600">Select all proxies</span>
                </div>
