...
// buildLocalFwdBin builds pgw-fwd (cmd/fwd) and returns the binary path.
func buildLocalFwdBin() (string, error) {
	goBin := ""
	for _, p := range []string{"/usr/local/go/bin/go", "/usr/bin/go", "/usr/local/bin/go"} {
		if _, e := os.Stat(p); e == nil { goBin = p; break }
	}
	if goBin == "" { return "", fmt.Errorf("go binary not found") }
	tmp, err := os.CreateTemp("", "pgw-fwd-*")
	if err != nil { return "", err }
	tmp.Close()
	cmd := exec.Command(goBin, "build", "-buildvcs=false", "-o", tmp.Name(), "./cmd/fwd")
	cmd.Dir = "/opt/proxy-server-local"
	cmd.Env = append(os.Environ(), "GOOS=linux", "GOARCH=amd64", "CGO_ENABLED=0",
		"PATH=/usr/local/go/bin:/usr/local/bin:/usr/bin:/bin", "HOME=/root",
		"GOCACHE=/tmp/pgw-go-cache", "GOPATH=/tmp/pgw-go-path")
	if out, err := cmd.CombinedOutput(); err != nil {
		_ = os.Remove(tmp.Name())
		return "", fmt.Errorf("build pgw-fwd: %w\n%s", err, out)
	}
	return tmp.Name(), nil
}

// generateAgentJWT creates a long-lived HS256 JWT for pgw-agent/pgw-fwd to authenticate against master API.
func generateAgentJWT(secret string) string {
	b64u := func(d []byte) string { return strings.TrimRight(base64.URLEncoding.EncodeToString(d), "=") }
	hdr, _ := json.Marshal(map[string]string{"alg": "HS256", "typ": "JWT"})
	pay, _ := json.Marshal(map[string]interface{}{
		"sub": "pgw-agent", "role": "agent",
		"exp": time.Now().Add(10 * 365 * 24 * time.Hour).Unix(),
	})
	msg := b64u(hdr) + "." + b64u(pay)
	mac := hmac.New(sha256.New, []byte(secret))
	mac.Write([]byte(msg))
	return msg + "." + b64u(mac.Sum(nil))
}
...
`
	...
	// Step 2c: Build and transfer pgw-fwd binary
	log("[deploy] Step 2c/5: Install pgw-fwd binary ...")
	if fwdBin, fwdErr := buildLocalFwdBin(); fwdErr != nil {
		log(fmt.Sprintf("[deploy] WARN: pgw-fwd build failed: %v — skipping", fwdErr))
	} else {
		defer os.Remove(fwdBin)
		log("[deploy] Transferring pgw-fwd binary via SSH ...")
		if err := transferBinary(client, fwdBin, "/usr/local/bin/pgw-fwd"); err != nil {
			log(fmt.Sprintf("[deploy] WARN: pgw-fwd transfer failed: %v", err))
		} else {
			log("[deploy] pgw-fwd binary transferred ✓")
		}
	}
	...
	// Node VPS cannot reach localhost or private IPs of the master — replace with outbound IP.
	isPrivate := func(url string) bool {
		return strings.Contains(url, "127.0.0.1") ||
			strings.Contains(url, "localhost") ||
			strings.Contains(url, "192.168.") ||
			strings.Contains(url, "10.") ||
			strings.HasPrefix(url, "http://172.")
	}
	if isPrivate(masterURL) {
		if ip := detectOutboundIP(); ip != "" {
			// Replace common private/loopback patterns with the detected outbound IP
			for _, priv := range []string{"127.0.0.1", "localhost", "192.168.1."} {
				if strings.Contains(masterURL, priv) {
					masterURL = strings.ReplaceAll(masterURL, priv, ip)
					break
				}
			}
			// If still private (different subnet), extract just the host:port pattern
			if isPrivate(masterURL) {
				// Parse port from current URL
				parts := strings.SplitN(strings.TrimPrefix(strings.TrimPrefix(masterURL, "http://"), "https://"), ":", 2)
				port := "8080"
				if len(parts) == 2 { port = parts[1] }
				scheme := "http"
				if strings.HasPrefix(masterURL, "https://") { scheme = "https" }
				masterURL = fmt.Sprintf("%s://%s:%s", scheme, ip, port)
			}
			log("[deploy] Master URL for node config: " + masterURL)
		}
	}
	// Generate a long-lived JWT for pgw-agent and pgw-fwd to call master API
	agentToken := generateAgentJWT(secret)
